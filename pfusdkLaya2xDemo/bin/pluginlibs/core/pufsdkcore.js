...
(function(PFU) {
    var WeChatIncentiveAd = function() {
        ...
        WeChatIncentiveAd.prototype.Show = function(service, fun) {
            var _this = this;
                ...
                if (typeof wx.createBannerAd === "function") {
                    ...
                    this._bannerAd.onResize(function(res) {
                        ...
                        if (dir == BannerDirction.DOWN_CENTER) {
                            ...
                            topPos = laya.utils.Browser.clientHeight - res.height;
                        }
...
(function(PFU) {
    var PfuPlatformManager = function() {
        ...
        PfuPlatformManager.prototype.GetShareQuery = function(sharePos) {
        ...
        PfuPlatformManager.prototype.LoginPlatform1003 = function(weToken, appId) {
            ...
            if (options) {
                var aid = PFU.WeChatUtils.GetInstance().GetAdAid();
                if (this.tempAdId != null) {
                    srcid = "weixinad" + this.tempAdId;
                } else if (aid != null || options.scene == 1045 || options.scene == 1046 || options.scene == 1067 || options.scene == 1084) {
                    srcid = "weixinad" + aid;
                } else if (options.scene == 1007 || options.scene == 1008 || options.scene == 1044 || options.scene == 1096) {
                    srcid = "share";
                } else if (options.scene == 1005 || options.scene == 1006 || options.scene == 1027 || options.scene == 1053) {
                    srcid = "search";
                } else {
    ...
    var PfuMoreGameUpdate = function() {
        ...
        PfuMoreGameUpdate.prototype.ShowMoreGameUI = function() {
            if (this.moreGameHandle) {
                this.moreGameFun.call(this.moreGameHandle, true);
            }
        };
        PfuMoreGameUpdate.prototype.HideMoreGameUI = function() {
            if (this.moreGameHandle) {
                this.moreGameFun.call(this.moreGameHandle, false);
            }
        };
        PfuMoreGameUpdate.prototype.CallShowMoreGameUI = function() {
...
(function(PFU) {
    ...
    var PfuManager = function() {
        ...
        PfuManager.prototype.PfuShare = function(key, fun) {
            this.PfuShareReplace(key, "", fun);
        };
        PfuManager.prototype.PfuShareNext = function(isShareGroup, replace, fun, qureyPos) {
            if (!this.IsExistShareData()) {
            ...
            var query = PFU.PfuPlatformManager.GetInstance().GetShareQuery(qureyPos);
        ...
        PfuManager.prototype.PfuShareReplace = function(key, replace, fun) {
            if (!this.IsExistShareData()) {
                fun(PfuSdk.FAIL);
                return;
            }
            var share = this.GetShareData(key);
            if (share == null) {
                fun(PfuSdk.FAIL);
                return;
            }
            ...
            PFU.WeChatUtils.GetInstance().ShareAppMessageImage(fun, str, imgUrl);
        };
...
(function(PFU) {
    var PfuGlobal = function() {
        ...
        PfuGlobal.ShowIncentive = function(service, fun) {
            PFU.WeChatIncentiveAd.GetInstance().Show(service, fun);
        };
        ...
        PfuGlobal.PfuShareGroupNext = function(handler, fun, isAward, qureyPos) {
            this._shareHandle(handler, fun, isAward, false, qureyPos);
        };
        PfuGlobal.PfuShareVideo = function(handler, fun, isAward, qureyPos) {
            this._shareHandle(handler, fun, isAward, true, qureyPos);
        };
        PfuGlobal._shareHandle = function(handler, fun, isAward, isVideo, qureyPos) {
            var _this = this;
            var stamp = Date.now();
            PFU.PfuManager.GetInstance().PfuShareNext(false, "", fun, qureyPos);
            if (isAward && !PFU.PfuManager.GetInstance().IsWegameTestMode()) {
                this.SetFocusHandler(this, function(time) {
                    var max = parseInt(_this.GetOLParam().pfuSdkShareTime) + 1e3 * PFU.PfuManager.GetInstance().GetShareTimeMax();
                    if (time - stamp >= max) {
                        console.log("分享成功");
                        fun.call(handler, PfuSdk.SUCCESS, "");
                        if (isVideo) {
                            PFU.PfuManager.GetInstance().AddVideoForceShareFinish();
                        }
                        PFU.PfuManager.GetInstance().AddShareCount();
                    } else {
                        var str = PFU.PfuManager.GetInstance().OLParam.pfuSdkShare1;
                        if (PFU.PfuManager.GetInstance().GetShareCount() > 0) {
                            str = PFU.PfuManager.GetInstance().OLParam.pfuSdkShare2;
                        }
...
(function(PFU) {
    var PfuBannerUpdate = function() {
        ...
        PfuBannerUpdate.prototype.Update = function() {
            ...
            this._timeCount += 1;
            if (this._timeCount > PFU.PfuGlobal.GetOLParam().banner_refresh_time) {
                this.RefreshBanner();
                this._timeCount = 0;
            }
        };
        ...
        PfuBannerUpdate.prototype.RefreshBanner = function() {
            var _this = this;
            this._isCreateBanner = false;
            if (PFU.PfuGlobal.GetOLParam().ad_banner == PFU.PfuSwitch.OFF) {
                this.RefreshPfuBanner();
...
var PfuSdk = function() {
    ...
    PfuSdk.Share = function(handle, qureyPos) {
        PFU.PfuGlobal.PfuShareGroupNext(handle, function() {}, false, qureyPos);
    };
    PfuSdk.ShareAward = function(handle, fun, qureyPos) {
        PFU.PfuGlobal.PfuShareGroupNext(handle, fun, true, qureyPos);
    };
    PfuSdk.Video = function(handle, fun) {
        var _this = this;
        if (PFU.PfuManager.GetInstance().IsVideoForceShare()) {
    ...
    PfuSdk.ShowMoreGameList = function() {
        PFU.PfuMoreGameUpdate.GetInstance().CallShowMoreGameUI();
    };
