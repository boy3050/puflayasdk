...
var MTA = {
  ...
  Page: {
    init: function () {
      ...
      MTA_CONFIG.stat_reach_bottom && a.onReachBottom && !
        function () {
    ...
    stat: function () {
      if ("" != MTA_CONFIG.app_id) {
        ...
        b = 0;
        for (var d = c.length; b < d; b++) for (var e in c[b]) c[b].hasOwnProperty(e) && a.push(e + "=" + ("undefined" == typeof c[b][e] ? "" : c[b][e]));
        wx.request({
  ...
  Event: {
    stat: function (a, b) {
      if ("" != MTA_CONFIG.event_id) {
        ...
        for (e = d.length; f < e; f++) for (var h in d[f]) d[f].hasOwnProperty(h) && c.push(h + "=" + ("undefined" == typeof d[f][h] ? "" : d[f][h]));
        wx.request({
